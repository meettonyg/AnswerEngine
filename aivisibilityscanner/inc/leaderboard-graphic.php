 ...
 */
function aivs_generate_leaderboard_svg( $entries, $title, $variant = 'social' ) {
	switch ( $variant ) {
		case 'blog':
			return aivs_leaderboard_svg_blog( $entries, $title );
		case 'presentation':
			return aivs_leaderboard_svg_presentation( $entries, $title );
		case 'social':
		default:
			return aivs_leaderboard_svg_social( $entries, $title );
	}
}
 ...
 */
function aivs_leaderboard_svg_social( $entries, $title ) {
	...
	return aivs_leaderboard_svg_render( $entries, $title, $width, $height, $row_h, $header_h );
}

/**
 * Blog variant — 1200x1600, top 20.
 */
function aivs_leaderboard_svg_blog( $entries, $title ) {
 ...
 */
function aivs_leaderboard_svg_presentation( $entries, $title ) {
	$entries  = array_slice( $entries, 0, 10 );
	$width    = 1920;
	$row_h    = 70;
	$header_h = 180;
 ...
 */
function aivs_leaderboard_svg_render( $entries, $title, $width, $height, $row_h, $header_h ) {
	$date = date( 'F j, Y' );
	$alt  = esc_attr( $title );
	...
	// Brand.
	$svg .= '<text x="60" y="50" font-family="Inter, Arial, sans-serif" font-size="16" font-weight="600" fill="#2563EB">AI Visibility Scanner</text>';
	...
	foreach ( $entries as $entry ) {
		...
		$row_y     = $y + ( $row_h / 2 ) + 5;
		$tier_color = isset( $entry['tier_color'] ) ? $entry['tier_color'] : aivs_get_tier_color( $entry['score'] );
	...
	// CTA.
	$cta_y = $height - 80;
	$svg .= '<text x="' . ( $width / 2 ) . '" y="' . $cta_y . '" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="14" fill="#2563EB">Run your own scan at aivisibilityscanner.com</text>';

	// Methodology note.
	$note_y = $height - 50;
	$svg .= '<text x="' . ( $width / 2 ) . '" y="' . $note_y . '" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="10" fill="#64748B">Rankings based on AI Visibility Scanner structural analysis. Scores reflect extractability signals, not endorsement or traffic.</text>';

	// Footer brand.
	$svg .= '<text x="' . ( $width / 2 ) . '" y="' . ( $height - 20 ) . '" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="11" fill="#64748B">aivisibilityscanner.com</text>';
