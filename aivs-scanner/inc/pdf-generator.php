...
function aivs_generate_pdf( $scan ) {
    ...
    $html = aivs_build_pdf_html( array(
        ...
        'layer_scores'  => $layer_scores ?: array(),
...
function aivs_build_pdf_html( $data ) {
    ...
    $layer_scores = isset( $data['layer_scores'] ) && is_array( $data['layer_scores'] ) ? $data['layer_scores'] : array();
    ...
    // Page 2: Stack Health
    if ( empty( $layer_scores ) ) {
        $layer_scores = aivs_calculate_layer_scores( is_array( $sub_scores ) ? $sub_scores : array() );
    }

    $stack_layers = array(
        array( 'num' => 1, 'name' => 'Access', 'key' => 'layer_1_access', 'coming_soon' => false ),
        array( 'num' => 2, 'name' => 'Understanding', 'key' => 'layer_2_understanding', 'coming_soon' => false ),
        array( 'num' => 3, 'name' => 'Extractability', 'key' => 'layer_3_extractability', 'coming_soon' => false ),
        array( 'num' => 4, 'name' => 'Trust', 'key' => '', 'coming_soon' => true ),
        array( 'num' => 5, 'name' => 'Authority', 'key' => '', 'coming_soon' => true ),
    );

    $html .= '<div class="page">
        <div class="brand">AI Visibility Scanner</div>
        <h2>Stack Health</h2>
        <p style="color:#64748B;margin-top:0;">Layer 3 is the citation gate: if extractability is weak, AI systems skip your content.</p>';

    foreach ( $stack_layers as $layer ) {
        if ( ! empty( $layer['coming_soon'] ) ) {
            $html .= '<div class="sub-score" style="opacity:.7;">
                <span class="sub-score-label">Layer ' . intval( $layer['num'] ) . ': ' . esc_html( $layer['name'] ) . '</span>
                <span class="sub-score-value" style="color:#94A3B8;">Coming soon</span>
            </div>';
            continue;
        }

        $score_value = isset( $layer_scores[ $layer['key'] ]['score'] ) ? intval( $layer_scores[ $layer['key'] ]['score'] ) : 0;
        $layer_tier = aivs_get_tier( $score_value );
        $html .= '<div class="sub-score">
            <span class="sub-score-label">Layer ' . intval( $layer['num'] ) . ': ' . esc_html( $layer['name'] ) . '</span>
            <span class="sub-score-value" style="color:' . esc_attr( $layer_tier['color'] ) . ';">' . $score_value . '/100</span>
        </div>';
    }
