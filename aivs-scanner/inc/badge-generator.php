 ...
 */
function aivs_generate_badge_svg( $data, $variant = 'inline' ) {
	...
	$domain = $url ? aivs_format_domain( $url ) : '';
	$alt_text = aivs_generate_alt_text( $domain ?: 'this site', $score, $label );

	switch ( $variant ) {
		case 'social':
			return aivs_badge_social( $score, $domain, $color, $label, $alt_text );
		case 'small':
			// Shorter truncation for the compact badge.
			$domain_small = $url ? aivs_format_domain( $url, 20 ) : '';
			return aivs_badge_small( $score, $domain_small, $color, $label, $alt_text );
		case 'inline':
		default:
			return aivs_badge_inline( $score, $domain, $color, $label, $alt_text );
	}
}
 ...
 */
function aivs_badge_inline( $score, $domain, $color, $label, $alt_text ) {
	$gauge_svg = aivs_score_gauge_svg( $score, 80, array(
		'stroke_width' => 7,
		'track_color'  => '#1E293B',
		'show_label'   => true,
	) );
	...
	// Attribution.
	$svg .= '<text x="120" y="120" font-family="Inter, Arial, sans-serif" font-size="9" fill="#64748B">aivisibilityscanner.com</text>';
 ...
 */
function aivs_badge_social( $score, $domain, $color, $label, $alt_text ) {
	...
	// Brand.
	$svg .= '<text x="600" y="80" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="18" font-weight="600" fill="#2563EB">AI Visibility Scanner</text>';
	...
	// CTA.
	$svg .= '<text x="600" y="530" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="14" fill="#2563EB">Scan yours free at aivisibilityscanner.com</text>';

	// Footer line.
	$svg .= '<line x1="400" y1="580" x2="800" y2="580" stroke="#1E293B" stroke-width="1"/>';
	$svg .= '<text x="600" y="600" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="11" fill="#64748B">aivisibilityscanner.com</text>';
 ...
 */
function aivs_badge_small( $score, $domain, $color, $label, $alt_text ) {
	...
	// Attribution.
	$svg .= '<text x="58" y="51" font-family="Inter, Arial, sans-serif" font-size="7" fill="#64748B">aivisibilityscanner.com</text>';
