 ...
 */
function aewp_format_domain( $url, $max_length = 30 ) {
	$domain = aewp_clean_url_for_display( $url );
 ...
 */
function aewp_generate_alt_text( $domain, $score, $tier_label ) {
	return sprintf(
 ...
 */
function aewp_score_gauge_svg( $score, $size = 100, $options = array() ) {
	$score        = max( 0, min( 100, intval( $score ) ) );
	$color        = aewp_get_tier_color( $score );
