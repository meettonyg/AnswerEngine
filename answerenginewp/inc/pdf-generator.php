    ...
    if ( ! empty( $competitor ) && is_array( $competitor ) ) {
        $html .= '<div class="page">
            ...
            <table class="comp-table">
                <tr><th>Signal</th><th>' . esc_html( $url ) . '</th><th>' . esc_html( $competitor['url'] ?? 'Competitor' ) . '</th></tr>
                <tr><td><strong>Overall Score</strong></td><td>' . $score . '</td><td>' . intval( $competitor['score'] ?? 0 ) . '</td></tr>';
        if ( ! empty( $competitor['sub_scores'] ) && is_array( $sub_scores ) ) {
            foreach ( $sub_scores as $key => $sub ) {
                if ( ! is_array( $sub ) ) continue;
                $comp_score = isset( $competitor['sub_scores'][ $key ]['score'] ) ? intval( $competitor['sub_scores'][ $key ]['score'] ) : '—';
                $html .= '<tr><td>' . esc_html( $sub['label'] ) . '</td><td>' . intval( $sub['score'] ) . '</td><td>' . $comp_score . '</td></tr>';
            }
        }
        $html .= '</table>
            <div class="footer-line">Generated by AnswerEngineWP &middot; answerenginewp.com</div></div>';
    }
