 ...
 */
function aewp_get_subscore_labels() {
	return array(
 ...
 */
function aewp_render_comparison_bars( $your_data, $competitor_data, $your_label = 'Your Site', $comp_label = 'Competitor' ) {
	...
	$labels      = aewp_get_subscore_labels();
	...
	// Overall score row.
	$html .= aewp_render_comparison_row( 'Overall Score', $your_overall, $comp_overall, true );
	...
	foreach ( $labels as $key => $label ) {
		...
		$html .= aewp_render_comparison_row( $label, $yours, $theirs, false );
	}
 ...
 * @return string HTML markup.
 */
function aewp_render_comparison_row( $label, $yours, $theirs, $is_overall = false ) {
	$delta     = $yours - $theirs;
 ...
 * @return string HTML markup.
 */
function aewp_render_before_after( $data ) {
	$before_score = intval( $data['before_score'] );
	$after_score  = intval( $data['after_score'] );
	$before_tier  = aewp_get_tier( $before_score );
	$after_tier   = aewp_get_tier( $after_score );
	$improvement  = $after_score - $before_score;
	$domain       = isset( $data['domain'] ) ? aewp_format_domain( $data['domain'] ) : '';
 ...
 */
function aewp_generate_comparison_svg( $your_data, $competitor_data, $your_label = 'Your Site', $comp_label = 'Competitor' ) {
	$your_scores = isset( $your_data['sub_scores'] ) ? $your_data['sub_scores'] : $your_data;
	$comp_scores = isset( $competitor_data['sub_scores'] ) ? $competitor_data['sub_scores'] : $competitor_data;
	$labels      = aewp_get_subscore_labels();
	...
	// Footer.
	$svg .= '<text x="' . ( $width / 2 ) . '" y="' . ( $total_h - 10 ) . '" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="9" fill="#64748B">answerenginewp.com</text>';
